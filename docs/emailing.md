Summary
1. Email tagging function (app/services/email.py)
add_load_board_tag(email, load_source) — adds +tag based on load source
Updated send_negotiation_email() to accept load_source and tag emails automatically
2. Load source tracking (app/schemas/load.py)
Added load_source: Optional[str] to LoadCreate schema
Tracks where each load came from (e.g., 'trucksmarter', 'dat')
3. Documentation (docs/EMAIL_TAGGING.md)
Strategy guide explaining how tagging works
How it works
Storage (clean base):
Store: gcs_parade@geodis.com (no tag)
Sending (dynamic tag):
...
Next steps
When you call send_negotiation_email(), pass load_source:
send_negotiation_email(
    to_email=broker.primary_email,  # Clean base from DB
    subject="...",
    body="...",
    load_id="...",
    negotiation_id=123,
    load_source="trucksmarter"  # ← Add this!
)
send_negotiation_email(    to_email=broker.primary_email,  # Clean base from DB    subject="...",    body="...",    load_id="...",    negotiation_id=123,    load_source="trucksmarter"  # ← Add this!)
The function will automatically tag the email so the broker's system sorts it correctly. Your directory stays clean, and emails are tagged based on load source.
